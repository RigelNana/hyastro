...
impl Inputs {
    const USAGE: &'static str = "usage: cargo run --features anise --example lunar_phases_year -- /path/to/de440s.bsp YEAR UTC_OFFSET_HOURS";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let year = Events::new(astrometry).moon_phase_year(
        inputs.year,
        offset,
    ...
    println!(
        "{} primary Moon phases at UTC{:+03}:00 (geocentric apparent lunar-solar longitude difference)",
        year.year(),
    ...
    for entry in year.entries() {
        ...
        println!(
            "{:02}-{:02} {:02}:{:02}:{:02}.{:09}  {:<13} D={:>6.1} deg  +/-{:.6} s",
            date.month(),
            date.day(),
            clock.hour(),
            clock.minute(),
